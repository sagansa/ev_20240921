...
class VerifyVehicleConnecting extends Command
{
    ...
    public function handle(): int
    {
        ...
        $modelsByKey = []; // "brandId|norm model" => model
        foreach (ModelVehicle::with('brandVehicle')->get() as $m) {
            $bId = $m->brand_vehicle_id;
            $modelsByKey[$bId.'|'.$norm($m->name)] ??= $m;
        }

        $typesByKey = []; // "modelId|norm type" => type
        foreach (TypeVehicle::all() as $t) {
            $typesByKey[$t->model_vehicle_id.'|'.$norm($t->name)] ??= $t;
        }

        // ---- Bandingkan tiap baris CSV ----
        $report = [
        ...
        while (($r = fgetcsv($handle)) !== false) {
            ...
            if ($type !== '' && $dbType === null) {
                $report['typeBaru'][] = ['brand' => $brand, 'model' => $model, 'type' => $type, 'pt' => $pt, 'category' => $category, 'size' => $size];
            }
            ...
            if ($diffs !== []) {
                ...
                $report['klasifikasiBeda'][$dKey] ??= ['brand' => $brand, 'model' => $model, 'diff' => implode('; ', $diffs)];
            }

            if ($diffs === [] && ($dbType !== null || $type === '')) {
                $report['match'][] = ['brand' => $brand, 'model' => $model, 'type' => $type];
            }
        }
        fclose($handle);

        // ---- Kebalikan: entitas DB yang tak direferensikan CSV ----
